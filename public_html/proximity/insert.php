...
if($cnt == 0) // if There is no other similar insertion then we add it to the Table
{
    ...
    $query = "INSERT INTO Proximity (userid, proximity_boolean, time_stamp) 
              VALUES ('$userId', '$proximity_boolean', TO_TIMESTAMP($timestamp)::TIMESTAMP)";
    ...
    $Idpairs =  "SELECT Users.id AS id_u, Proximity.id AS id_m 
    FROM Users, Proximity
    WHERE Users.userid = Proximity.userid AND Users.userid = '$userId' AND Proximity.userid = '$userId'";   
    ...
    $insertMeasurement = "INSERT INTO Proximity_measurement (id_Users,id_Proximity)
    ...
    pg_close($dbConn);
    //header("Locaiton: ../proximity_sensor.php");
    exit;
}
else
   {

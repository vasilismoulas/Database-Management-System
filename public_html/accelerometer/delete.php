...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "DELETE FROM Accelerometer
              WHERE userid = '$userId'
                AND accelerometer_x = CAST('$accelerometerX' AS double precision)
                AND accelerometer_y = CAST('$accelerometerY' AS double precision)
                AND accelerometer_z = CAST('$accelerometerZ' AS double precision)
    ...
    pg_close($dbConn);
    exit;
    //header("accelerometer_sensor.php");
}
?>
